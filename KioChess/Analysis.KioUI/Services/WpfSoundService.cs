    ...
    private void LoadSoundResources()
    {
        ...
        foreach (var name in soundNames)
        {
            try
            {
                ...
                var uri = new Uri($"pack://application:,,,/Analysis.KioUI;component/{resourcePath}");
        ...
        Task.Run(() =>
        {
            try
            {
                if (_soundStreams.TryGetValue(name, out var stream) && stream != null)
                {
                    ...
                    // This allows multiple simultaneous sounds
                    ...
                    // Use SoundPlayer for simple WAV playback
                    using var player = new SoundPlayer(playbackStream);
                    player.PlaySync(); // Plays in background thread, doesn't block UI
                }
            }
            catch
            {
