...
namespace Engine.Strategies.Base
{
    public abstract partial class StrategyBase
    {
        ...
        {
            Sorters = new MoveSorterBase[SortDepth[depth][2]];
            ...
            for (int i = 1; i < SortDepth[depth][0]; i++)
            {
                Sorters[i] = mainSorter;
            }
            for (var i = SortDepth[depth][0]; i < SortDepth[depth][1]; i++)
            {
                Sorters[i] = initialSorter;
            }
            for (var i = SortDepth[depth][1]; i < SortDepth[depth][2]; i++)
            {
