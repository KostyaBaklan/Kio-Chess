...
/// <summary>
/// ViewModel for the Opening Explorer tab.
/// Provides interactive opening book exploration with tree navigation.
/// </summary>
public class OpeningExplorerViewModel : BindableBase
{
        ...
        Position position)
    {
        ...
        Board = new BoardViewModel { BoardState = Models.BoardState.ReadOnly };
        PossibleMoves = new ObservableCollection<OpeningMoveViewModel>();
        SearchResults = new ObservableCollection<OpeningSearchResultViewModel>();
    ...
    private async void OnSearch()
    {
        ...
        ShowSuggestions = false; // Hide suggestions when showing full results
    ...
    private async Task UpdateSearchSuggestionsAsync()
    {
        ...
        try
        {
            // Get more results for prioritization
            var results = await _explorerService.SearchByNameAsync(SearchQuery, 50);
    ...
    /// <summary>
    /// Calculate search relevance score for prioritizing results.
    ...
    private int CalculateSearchScore(OpeningEntry opening, string query)
    {
        ...
        // 1. ECO code matches (highest priority)
        ...
        // 3. Full name matches (includes variations)
        ...
        // 4. Variation matches (third priority)
        if (!string.IsNullOrEmpty(opening.Variation))
        {
            ...
            if (variationLower.StartsWith(queryLower))
                score += 80;
        ...
        // Bonus: Popularity boost (small influence)
        score += opening.Popularity / 10;
        
        // Bonus: Prefer shorter move counts (more fundamental openings)
    ...
    private async void OnJumpToOpening(OpeningSearchResultViewModel result)
    {
        ...
        try
        {
            ...
            // Replay all moves to reach this opening
    ...
    private async void OnSelectSuggestion(string suggestion)
    {
        ...
        try
        {
            ...
            // Extract ECO code if present (format: "[ECO] Name")
            var query = suggestion.StartsWith("[") 
                ? suggestion.Substring(suggestion.IndexOf(']') + 2).Trim()
                : suggestion;

            // Search for exact opening and jump to it
...
/// <summary>
/// ViewModel for a possible next move in the opening tree.
/// </summary>
...
/// <summary>
/// ViewModel for opening search results.
/// </summary>
