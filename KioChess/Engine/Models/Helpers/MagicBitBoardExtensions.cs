...
public static class MagicBitBoardExtensions
{
    ...
    [InlineArray(512)]
    private struct BishopDB
    {
        public BitBoard Db;
    }

    [InlineArray(4096)]
    private struct RookDB
    {
        public BitBoard Db;
    }

    private static readonly CellBuffer<BishopDB> _magicBishopDb = new();
    private static readonly CellBuffer<RookDB> _magicRookDb = new();

    private static readonly CellBuffer<ulong> _magicMovesBishopMagics;
    private static readonly CellBuffer<ulong> _magicMovesBishopMask;
    private static readonly CellBuffer<ulong> _magicmovesRookMagics;
    private static readonly CellBuffer<ulong> _magicmovesRookMask;

    static MagicBitBoardExtensions()
    {
        for (var i = 0; i < 64; i++)
            _magicBishopDb[i] = new();
        ...
        ulong[] magicMovesBMagics =
       {
        ...
        {
            _magicMovesBishopMagics[i] = magicMovesBMagics[i];
            _magicMovesBishopMask[i] = magicMovesBMask[i];
            _magicmovesRookMagics[i] = magicmovesRMagics[i];
            _magicmovesRookMask[i] = magicmovesRMask[i];
            numSquares = 0;
            var temp = _magicMovesBishopMask[i];
            while (temp != 0)
            {
        ...
        {
            numSquares = 0;
            var temp = _magicmovesRookMask[i];
            while (temp != 0)
            {
                var bit = (ulong)((long)temp & -(long)temp);
                squares[numSquares++] = initMagicMovesDb[(int)((bit * 0x07EDD5E59A4E28C2UL) >> 58)];
